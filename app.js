...
async function loadSoundFontLib() {
  ...
  // SoundBankLoader belongs to spessasynth_core, not spessasynth_lib.
  // spessasynth_lib provides the browser/WebAudio WorkletSynthesizer wrapper.
  const [core, lib] = await Promise.all([
    import("https://cdn.jsdelivr.net/npm/spessasynth_core@4.3.0/+esm"),
    import("https://cdn.jsdelivr.net/npm/spessasynth_lib@4.3.0/+esm")
  ]);
  SoundBankLoader = core.SoundBankLoader;
  WorkletSynthesizer = lib.WorkletSynthesizer;
  if (!SoundBankLoader) throw new Error("SoundBankLoader could not be loaded from spessasynth_core.");
