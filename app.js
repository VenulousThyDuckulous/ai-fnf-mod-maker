...
// spessasynth_lib already accepts raw SF2/SF3/DLS ArrayBuffers through
// soundBankManager.addSoundBank(). Do NOT import SoundBankLoader from
// spessasynth_lib: it is not one of that package's exports.
async function loadSoundFontLib() {
  if (WorkletSynthesizer) return;
  const lib = await import("https://cdn.jsdelivr.net/npm/spessasynth_lib@4.3.0/+esm");
...
$("sf2").onchange = async e => {
  ...
  try {
    ...
    // Do not parse with SoundBankLoader here. The browser synth accepts the
    // raw ArrayBuffer directly, which avoids the incorrect export/API issue.
    // The preset selector is kept as a MIDI program selector (0-127).
    $("preset").innerHTML = "";
    for (let i = 0; i < 128; i++) {
      const o = document.createElement("option");
      o.value = i;
      o.textContent = `MIDI ${i}`;
      $("preset").appendChild(o);
    }
    selectedPreset = 0;
    synth = { soundBank: null, ready: false };
    $("sfStatus").textContent = `Loaded ${f.name} • SF2/SF3 data ready`;
  } catch (e) {
    console.error(e);
...
$("testNote").onclick = async () => {
  ...
  try {
    ...
    synth.engine.programChange(0, selectedPreset);
