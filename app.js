...
let SoundBankLoader = null, WorkletSynthesizer = null, audioContext = null;
...
async function loadSoundFontLib() {
  if (SoundBankLoader && WorkletSynthesizer) return;
  // v3 used the old SoundFont API. Current spessasynth_lib uses
  // SoundBankLoader.fromArrayBuffer() and WorkletSynthesizer.
  const mod = await import("https://cdn.jsdelivr.net/npm/spessasynth_lib@4.3.0/+esm");
  SoundBankLoader = mod.SoundBankLoader;
  WorkletSynthesizer = mod.WorkletSynthesizer;
  if (!SoundBankLoader) throw new Error("SoundBankLoader was not exported by spessasynth_lib.");
...
$("sf2").onchange = async e => {
  ...
  try {
    ...
    const sf = SoundBankLoader.fromArrayBuffer(sfData);
    if (sf.isSF3DecoderReady) await sf.isSF3DecoderReady;
    ...
    // Keep the parsed bank around. The browser synth is initialized lazily
    // when the user asks for a preview note.
    synth = { soundBank: sf, ready: false };
...
$("testNote").onclick = async () => {
  ...
  try {
    if (!audioContext) {
      ...
      const workletURL = "https://cdn.jsdelivr.net/npm/spessasynth_lib@4.3.0/dist/spessasynth_processor.min.js";
    ...
    synth.engine.programChange(0, Number(synth.soundBank.presets[selectedPreset]?.program ?? selectedPreset));
...
$("export").onclick = async () => {
  if (!chart) return;
  if (typeof JSZip === "undefined") throw new Error("JSZip failed to load. Refresh the page and try again.");
